...
static void
create_tree (HTMLObject *obj, AtkObject *parent, gint child_index)
{
	AtkObject *added = NULL;
	AtkObject *new_parent = parent;

	switch (HTML_OBJECT_TYPE (obj)) {
	case HTML_TYPE_CLUEFLOW:
		new_parent = added = html_a11y_paragraph_new (obj);
		break;
		/* case HTML_TYPE_TEXT:
		added = gtk_html_a11y_text_new (obj, parent);
		break;
	case HTML_TYPE_LINKTEXT:
		added = gtk_html_a11y_link_new (obj, parent);
		break;
	case HTML_TYPE_IMAGE:
		added = gtk_html_a11y_image_new (obj, parent);
		break;
	case HTML_TYPE_TABLE:
		new_parent = added = gtk_html_a11y_table_new (obj, parent);
		break; */
	...
	if (added) {
		atk_object_notify_state_change (added, ATK_STATE_VISIBLE, TRUE);
		atk_object_notify_state_change (added, ATK_STATE_ENABLED, TRUE);
		atk_object_set_parent (added, parent);
		g_signal_emit_by_name (parent, "children_changed::add", child_index, added, NULL);
	}

	if (html_object_is_container (obj)) {
		HTMLObject *cur;

		cur = html_object_head (obj);
		child_index = 0;
		while (cur) {
			create_tree (cur, new_parent, child_index);
